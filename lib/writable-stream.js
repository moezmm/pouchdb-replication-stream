...
function WritableStreamPouch(opts, callback) {
    ...
    function transformDocsAttachments(docs) {
        return new Promise((resolve, reject) => {
                let arrayPromises = [];
                docs.forEach(function (doc) {
                    if ((null !== doc.attachments) && doc._attachments) {
                        Object.keys(doc.attachments).forEach(function (attachment, index) {

                            //We assume that attachments order is coherent --> doc._attachments[i] is the file attachment referenced by doc.attachments[i]
                            //TODO: Use bigInt library to adress the right attachment key (20 characters to int)
                            if (doc._attachments[Object.keys(doc._attachments)[index]].data) {
                                arrayPromises.push(promiseblobToDataURL(
                                    doc._attachments[Object.keys(doc._attachments)[index]].data).then(newDataUrl => {
                                        doc._attachments[Object.keys(doc._attachments)[index]].data = newDataUrl.split(',')[1];
                                    })
                                );
    ...
    api._bulkDocs = function (req, opts, callback) {
        ...
        if (opts.new_edits === false) {
            ...
            transformDocsAttachments(docs)
                .then(docs => {
                    self.ndj.write({docs: docs}, function () {
